...
var VoicesApp = angular.module('VoicesApp', [
  'ngFileUpload', 'uiSwitch', 'angularModalService'
]);
...
VoicesApp.service('pageClassService', 
  function () {
    this.getPageClass = function() {
      ...
      if (url.indexOf('/annotate') > -1) currentPageClass = 'annotate';
      if (url.indexOf('/records') > -1) currentPageClass = 'show';
      if (url.indexOf('/new') > -1) currentPageClass = 'new';
      if (url.indexOf('/edit') > -1) currentPageClass = 'edit';
...
VoicesApp.factory('httpService', [ '$http',
  function ($http) {
    return {
...
VoicesApp.service('postRecordForm', [ '$http',
  function ($http) {
    this.uploadForm = function(form){
      
      // retrieve the CSRF token to we can make the POST request
      // without getting 422'd
      ...
      */
      ...
      fd = new FormData;
      for (var k in form) fd.append('record[' + k + ']', form[k]);
      $http({
        method: 'POST',
        url: myRecordUrl,
        headers: {
...
VoicesApp.controller('FormController', [
    ...
    'postRecordForm', 'pageClassService',
  function($scope, $http, $location, Upload, postRecordForm, pageClassService ) {
    ...
    $scope.getAttachments = function(recordId) {
      if (recordId) { $http.get('/records/' + recordId + '.json')
        // on success
    ...
    if (url.indexOf('/edit') > -1) url = url.replace('/edit', '');
    // then parse the record id
    if (url.indexOf('/records/') > -1) {
      var split = url.split('records/')[1].split('?')[0].split('.')[0];
      var recordId = Number( split );
    }
    ...
    $scope.aws = {
      'AWSAccessKeyId': 'AKIAI5CWU3VZSWDIGR7Q',
      'policy': '',
      'signature': 'fBswBEoFuH7aaGWAr3IrvbBJgh0=',
      'acl': 'public-read',
      'rooturl': 'https://lab-file-uploads.s3.amazonaws.com/'
    };
    ...
    var sendFileToAWS = function(file, filesize, timestamp) {
      ...
      $scope.abort = function(fileInTransit) {        
        ...
        for (i = 0; i < $scope.imageSizes.length; i++) {
          ...
          if (index > -1) $scope.filesToSend.splice(index, 1);
          var sentIndex = $scope.filesSent.indexOf(
            timestampedFilename + filesize);
          if (sentIndex > -1) $scope.filesSent.splice(sentIndex, 1);
    ...
    $scope.getProgress = function(f) {
      var progress = 0;
      if (f.progress.original) progress += f.progress.original;
      ...
      if (f.file.type.indexOf('image') > -1) {
        for (i=0; i<$scope.imageSizes.length; i++) {
          ...
          if (filesize != 'original') {
            ...
            if (sizeProgress) progress += sizeProgress;
          }
        };
        return (progress/4);
      }
...
VoicesApp.controller('ModalController', [
    '$scope', '$http', '$element', 'close',
    'attachment', 'saveAnnotationService',
  function($scope, $http, $element, close,
    attachment, saveAnnotationService) {
    ...
    var hasPlaceholderImage = function(attachment) {
      var path = attachment.placeholder_image_path;
...
VoicesApp.controller('GalleryController', [
    '$scope', '$http', '$location', 'ModalService',
    'saveAnnotationService', 'pageClassService',
  function($scope, $http, $location, ModalService,
    ...
    $scope.getRecordId = function() {
      ...
      if (url.indexOf('/records/') > -1) {
        var split = url.split('records/')[1].split('?')[0].split('.')[0];
        $scope.recordId = Number( split );
      } 
      if (url.indexOf('/annotate/') > -1) {
        var split = url.split('annotate/')[1].split('?')[0].split('.')[0];
        $scope.recordId = Number( split );
      }
    ...
    $scope.setAttachmentsPerPage = function() {
      var url = window.location.href;
      // the gallery view should hold 20 images per page for all views except
      // annotate, iff there are more than n attachments for the record
      if (url.indexOf('/annotate') > -1) {
        // if the user has only one record, push 1 attachment per page to
        // client; nb: this should never happen through normal operations,
        // as users are only requested to annotate a record's annotations
        // if that record has > 4 attachments, but if a user requests
        // ROOTURL/annotate/Record-with-1-attachment-id, we want to serve
        // them a nice looking page
      ...
      if ($scope.data.attachments.length > $scope.superUserBreakpoint) {
        $scope.attachmentsPerPage = $scope.attachmentsPerPage ?
          $scope.attachmentsPerPage : 20;
      }
      // use ternary operator to finalize attachments per page
      $scope.attachmentsPerPage = $scope.attachmentsPerPage ?
        $scope.attachmentsPerPage : 4;
      // store the initial attachmentsPerPage for restoring
      // attachmentsPerPage on view toggle
    ...
    $scope.setTotalNumberOfPages = function() {
      ...
      var num = $scope.data.attachments.length;
      var denom = $scope.attachmentsPerPage;
      $scope.totalNumberOfPages = Math.ceil( num / denom );
    };
    ...
    $scope.getAttachments = function(recordId, pageToFetch) {
      if (recordId) {
        $http.get('/records/' + recordId + '.json').then(function(response) {
          // on success
    ...
    $scope.hasPlaceholderImage = function(attachment) {
      var path = attachment.placeholder_image_path;
      if (path !== null) return attachment.placeholder_image_path;
      return false;
    };
    ...
    $scope.getImageUrl = function(attachment) {
      ...
      if ($scope.attachmentsPerPage == 1) {
        assetPath = attachment.medium_image_url;
      }
      if ($scope.attachmentsPerPage == 4) {
        assetPath = attachment.annotation_thumb_url;
      }
      if ($scope.attachmentsPerPage == 20) {
        assetPath = attachment.square_thumb_url;
      }
      return assetPath;
    };
